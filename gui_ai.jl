...
function hardcoreAI(AI::MonteCarlo, set::Pieces, enemy::Pieces)
  set.color == "black" ? play = black_MCTS(AI) : play = red_MCTS(AI)
  moveAI(set,enemy,play[2][1],play[2][2])
end
...
function suicideAI(set::Pieces, enemy::Pieces, piece::ASCIIString)
    ...
    if haskey(set.activeS,target) != true
        move_piece(set,red,piece,target) 
    else
...
function randomAI(set::Pieces, enemy::Pieces)
    ...
    while length(legal) == 0
        piece = rand(collect(keys(black.active)))
...
function place_piece(B::GUI, i::ASCIIString, x::Int, y::Int)
    ...
    function isSecondClick(path)
        ...
        elseif B.clicks == 2 && double_list(B.piece) == 0
            singleStep(B,x,y)
            B.first == "AI" ? turnAI(B,black,red) : turnAI(B,red,black)
        elseif B.clicks == 2
...
if difficulty == "Hardcore"
  for i = 1:len
    get!(julia.history,stats[i],stats[i,2])
  end
end
...
elseif first_player == "AI"
  time_limit != -1 && (B.time = Int(datetime2unix(now())))
  turnAI(B,black,red)
  setBoard(B,black,red)
end
