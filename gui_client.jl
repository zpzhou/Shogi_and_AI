...
function nextMove(file::ASCIIString, black::Pieces, red::Pieces)
    ...
    replay_piece(red,black,piece,target,promote)
    target2 != (0,0) && replay_piece(red,black,piece,target2,promote)
    target3 != (0,0) && replay_piece(red,black,piece,target3,promote)
end
...
function singleStep(B::GUI, x::Int64, y::Int64)
    ...
    if flag == 1
        ...
        haskey(black.active,ucfirst(B.piece)) ? promote = 1 : promote = 0
        set_move(file,"move",B.source,target,(0,0),(0,0),promote,0,"")

        move_user_server_game(ip,port,file)
        setBoard(B,black,red)
        nextMove(file,black,red)
        setBoard(B,black,red)
