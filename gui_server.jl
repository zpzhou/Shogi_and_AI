...
function checkWinMessage(B::GUI, black::Pieces, red::Pieces)
    ...
    elseif B.time != -1
        if get_senteTime(file) <= 0
            message = "Black ran out of time, Red Wins"
            Messagebox(window,message)
        elseif get_goteTime(file) <= 0
            message = "Red ran out of time, Black Wins"
            Messagebox(window,message)
        end
    end
end
...
function setBoard(B::GUI, black::Pieces, red::Pieces)
    println("Entering setBoard function")
    len = B.dim
    for y in 1:len
        for x in 1:len
            place_piece(B,"", x,y)
    ...
    B.updated = 0
    println("exitting setBoard function")
end
