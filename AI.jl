...
function king_AI(set::Pieces, legal::Array, piece::ASCIIString)
    ...
    if x != 9 && y != 9 # step top right
        haskey(friends,(x+1,y+1)) == 0 && push!(legal,(x+1,y+1))
    end
    if x != 1 && y != 9 # step top left
        haskey(friends,(x-1,y+1)) == 0 && push!(legal,(x-1,y+1))
    end
    if x != 9 && y != 1 # step bottom right
        haskey(friends,(x+1,y-1)) == 0 && push!(legal,(x+1,y-1))
    end
...
# randomly generates a legal drop coordinated for a given piece
...
function getPlay(AI::MonteCarlo, set::Pieces, enemy::Pieces)
    ...
    for i = 1:length(A)
        legal = generate_moves(set,enemy,A[i])
...
# # file = readdlm("MCTS.txt"); len = size(file)[1]
# # for i = 1:len
...
# # #simulate(julia)
# for i = 1:10000 # run simulations to fill MCTS
#     simulate(julia)
# end

# # add in new history
# writedlm("MCTS.txt",julia.history)

# A = collect(keys(julia.history))
# for i = 1:length(A)
#     println(julia.history[A[i]])
# end
#println(length(A))
